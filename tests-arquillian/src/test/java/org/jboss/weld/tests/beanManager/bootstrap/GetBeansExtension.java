...
public class GetBeansExtension implements Extension {
    ...
    private boolean hasBeanFromManager;

    public boolean hasBeanFromManager() {
        return hasBeanFromManager;
    }
    ...
    private void afterBeanDiscovery(@Observes AfterBeanDiscovery abd, BeanManager manager) {
        if (!manager.getBeans(CdiExtensionBean.class).isEmpty()) {
            hasBeanFromManager = true;
        }
    }
}
