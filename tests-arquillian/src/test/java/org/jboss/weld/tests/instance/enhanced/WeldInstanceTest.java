...
public class WeldInstanceTest {
    ...
    @Test
    public void testHandlerIterator(WeldInstance<Processor> instance) {
        ActionSequence.reset();
        assertTrue(instance.isAmbiguous());
        for (Iterator<Handler<Processor>> iterator = instance.handlerIterator(); iterator.hasNext();) {
            Handler<Processor> processor = iterator.next();
            processor.get().ping();
        ...
        assertTrue(instance.isAmbiguous());
        for (Iterator<Handler<Processor>> iterator = instance.handlerIterator(); iterator.hasNext();) {
            try (Handler<Processor> processor = iterator.next()) {
                processor.get().ping();
            }
        }
