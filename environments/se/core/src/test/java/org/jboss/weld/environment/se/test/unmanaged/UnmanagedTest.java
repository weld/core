...
public class UnmanagedTest {
    ...
    public static Archive<?> getDeployment() {
        return ShrinkWrap.create(WeldSEClassPath.class).add(
                ShrinkWrap.create(BeanArchive.class).addClasses(Foo.class, Bar.class, UnmanagedTest.class));
    }

    @Test
    public void testUnmanaged() {
        Unmanaged<Foo> foo = new Unmanaged<Foo>(Foo.class);
        UnmanagedInstance<Foo> instance = foo.newInstance();
        instance.produce().inject().postConstruct();
        Assert.assertNotNull(instance.get().getBar());
