...
class Probe {
    ...
    Probe() {
        ...
        beanToDeclaredProducers = HashMultimap.create();
    ...
    Set<AbstractProducerBean<?, ?, ?>> getDeclaredProducers(Bean<?> bean) {
        return (Set<AbstractProducerBean<?, ?, ?>>) (beanToDeclaredProducers.containsKey(bean) ? beanToDeclaredProducers.get(bean) : Collections.emptySet());
    }
