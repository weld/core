...
`Instance.getHandle()` returns an `Instance.Handle<T>`, which provides access to the contextual reference through `get()`, bean metadata through `getBean()`, and explicit destruction through `destroy()`.
Handles also implement `AutoCloseable` and can be used in a try-with-resources statement:
...
class Foo {
  ...
  void doWork() {
    try (Instance.Handle<Bar> handle = instance.getHandle()) {
      handle.get().doBusiness();
    }
  }
...
`Instance.handles()` and `Instance.handlesStream()` provide iteration and streaming over handles for all matching beans.
These are standard CDI APIs and do not require a dependency on the Weld API.

==== Enhanced version of `jakarta.enterprise.inject.Instance`

Weld provides `org.jboss.weld.inject.WeldInstance`, which extends `Instance` with selection by `java.lang.reflect.Type` and `getHandlePriorityComparator()`.
The comparator sorts handles by descending bean priority and can be passed to `handlesStream().sorted(instance.getHandlePriorityComparator())`.

Weld's additional `select()` overload accepts `java.lang.reflect.Type` as a parameter and optionally qualifier(s).
