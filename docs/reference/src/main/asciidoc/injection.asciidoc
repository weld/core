...
Weld also provides `org.jboss.weld.inject.WeldInstance` - an enhanced version of `javax.enterprise.inject.Instance`.
There are four additional methods.
...
Another addition is a convenient method `isResolvable()` - a replacement for `!isUnsatisfied() && !isAmbiguous()` which is the expression most users are interested in:
...
Last method is a twist on the `select()` method, but it accepts `java.lang.reflect.Type` as parameter and optionally qualifier(s).
