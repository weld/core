...
=== Accessing interceptor bindings

Jakarta Interceptors provides `InvocationContext.getInterceptorBindings()` to obtain all bindings and `getInterceptorBindings(Class<T>)` to obtain bindings of a given type.
This is useful for reading `@Nonbinding` members, whose values do not affect interceptor selection.
For example, an interceptor can read a `FooBinding` annotation's `@Nonbinding String secret()` member as follows:

[source.JAVA, java]
-------------------------------------------------------------------------------------------------
@AroundInvoke
Object intercept(InvocationContext ctx) throws Exception {
    for (FooBinding binding : ctx.getInterceptorBindings(FooBinding.class)) {
        String secret = binding.secret();
        // Use the value configured on the intercepted bean or method.
    }
    return ctx.proceed();
}
-------------------------------------------------------------------------------------------------
